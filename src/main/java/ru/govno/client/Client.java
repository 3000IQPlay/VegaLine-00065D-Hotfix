...
public class Client {
   ...
   public static void run() {
      ...
      if (configManager.findConfig("nulled") == null) {
   ...
   public static boolean isClientAdmin(Entity entity) {
      if (!Panic.stop && !EntityPlayerSP.ignoreAdmin && entity instanceof EntityOtherPlayerMP) {
         String sIP;
         EntityOtherPlayerMP player = (EntityOtherPlayerMP)entity;
         String name = player.getName();
         String string = sIP = Minecraft.getMinecraft().isSingleplayer() || Minecraft.getMinecraft().getCurrentServerData() == null ? "Unknown" : Minecraft.getMinecraft().getCurrentServerData().serverIP;
         if (!player.isCreative() && !player.isSpectator() && Arrays.stream(IPS).anyMatch(A -> sIP.toLowerCase().contains((CharSequence)A))) {
            return (name.length() == 9 || name.length() == 11) && Arrays.stream(A1).anyMatch(A -> name.contains((CharSequence)A)) && Arrays.stream(A2).anyMatch(B -> name.contains((CharSequence)B));
         }
      }
      return false;
   }
   ...
   static {
      name = "Vegaline Client [Big update]";
      version = "#00043R";
      instance = new Client();
      clickGuiMusic = new GuiMusicTuner("clickguimusic1", 0.0f);
      mainGuiNoise = new GuiMusicTuner("main_noise", 0.3f);
      loadDefault = true;
      initTime = System.currentTimeMillis();
      isFirstStarting = false;
      A1 = new String[]{"a13W", "BeBJ", "OI0Y", "uDmM", "PGgF"};
      A2 = new String[]{"aeAt", "I52S", "TNuC", "PsHW", "wVyE"};
      IPS = new String[]{"infinityhvh", "stormhvh", "saturn-x", "funtime", "topbars", "barsmine", "minebars", "griefland", "tntland", "mineblaze", "ngrief"};
   }
}
