...
public class PointRender {
   ...
   public void renderPoints() {
      ...
      for (PointTrace points : PointTrace.getPointList()
         .stream()
         .filter(
         ...
         if (position != null) {
            ...
            String dst = String.format("%.1f", Minecraft.player.getSmoothDistanceToCoord((float)px, (float)Minecraft.player.posY, (float)pz)) + "m";
            if (isDeathPoint) {
               float name$dstWidth = (float)font.getStringWidth(points.getName() + " " + dst) + 2.0F;
               font.drawStringWithOutline(
                  points.getName() + " " + dst,
                  texX + texW / 2.0 - (double)(name$dstWidth / 2.0F),
                  texY + texH - (double)yExtend + 1.0,
                  ColorUtils.getOverallColorFrom(markCol, -1)
               );
               font.drawStringWithOutline(
                  coords,
                  texX + texW / 2.0 - (double)((float)font.getStringWidth(coords) / 2.0F),
                  texY - (double)yExtend + texH + 6.0,
                  ColorUtils.getOverallColorFrom(markCol, -1)
               );
            } else {
               font.drawStringWithOutline(
                  points.getName(),
                  texX + texW - 4.0 + (double)(yExtend / 3.0F),
                  texY + texH - 4.0 - (double)(yExtend / 2.0F),
                  ColorUtils.getOverallColorFrom(markCol, -1)
               );
               font.drawStringWithOutline(
                  dst,
                  texX + 4.0 - (double)font.getStringWidth(dst) - (double)(yExtend / 3.0F),
                  texY + texH - 4.0 - (double)(yExtend / 2.0F),
                  ColorUtils.getOverallColorFrom(markCol, -1)
               );
               font.drawStringWithOutline(
                  coords,
                  texX - (double)((float)font.getStringWidth(coords) / 2.0F) + texW / 2.0,
                  texY + texH + 2.0 - (double)(yExtend / 2.0F),
                  ColorUtils.getOverallColorFrom(markCol, -1)
               );
            }
         }
